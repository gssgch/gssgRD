...
object MovieRecommend {
  ...
  def main(args: Array[String]) {
    ...
    // 将样本评分表以key值切分成3个部分，60%用于训练 (加入用户评分), 20%用于校验, 剩下20%用于测试。由于这些数据在计算过程中要多次用到，所以cache到内存
    val numPartitions = 4
    val training = ratings.filter(x => x._1 < 6) // 60%的训练数据
      .values
    ...
    // 下面开始训练不同参数下的模型，并在校验集中验证，获取最佳参数下的模型
    val ranks = List(8, 12)
    ...
    // 使用上面训练出的最佳模型预测测试集的评分，并计算和实际评分之间的均方根误差
